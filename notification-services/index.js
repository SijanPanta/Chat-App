...
const sendWelcomeEmail = async (userData) => {
  try {
    console.log(`[Notification Service] 📧 Processing welcome email for: ${userData.email}`);
    ...
    console.log(`[Notification Service] ✅ Email actually sent: ${info.messageId}`);
    

    // Simulating delay for async processing
    await new Promise((resolve) => setTimeout(resolve, 1000));
    console.log(`[Notification Service] ✅ Successfully handled email for ${userData.username}`);
  } catch (error) {
...
const connectAndConsume = async () => {
  try {
    ...
    console.log(`🚀 [Notification Service] Connected and listening on queue: '${QUEUE_NAME}'`);
    ...
    channel.consume(QUEUE_NAME, async (msg) => {
      if (msg !== null) {
        try {
          ...
          channel.ack(msg);
          console.log(`[Notification Service] 🧹 Message ACKed and removed from queue.`);
    ...
    console.error("❌ [Notification Service] Failed to connect to RabbitMQ:", error.message);
